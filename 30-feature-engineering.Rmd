...
Below, I create a function called scale2 that standardizes the values given to it around a mean value of 0. I then use a scoped mutate to apply the function to all observations in the eccentricty and period_minutes columns, and create new columns that contain the standardized values. This will help pick out any outliers and any possible clusters that form when looking at that particular data values. Based on that, some information could be extracted to help evaluate the purpose of the satellite given that information. 
...
The second engineered feature that I am adding to this dataset is a column that uses the information given in the date_of_launch column and provides the day of the week the launch took place. This is done using the wday function from the lubridate library. The created function dayoflaunch will be applied to the given column using a scoped mutate, to create a new column called Launchday. The function takes in a parameter (value from column) and converts it to the day of launch (value of 1-7).
